...
{
  for (int i = 0; i < 3; i++){
/// na verdade, %255 vai até 254. para ir até 255, tem que ser %256
...
/// esse código é quase igual ao de cima, uma função com isso limparia o código
    for (int i = 0; i < 5; i++){
      pos(4+i, 40);
      for(int j = 0; j < 15; j++){
...
/// deve ser possível dar uma simplificada nisso...
